...
const Alexa2Auth = require("../lib/authEng/auth"); // => auth.js
...
module.exports = function (RED) {
        function AlexaRemoteAccountNode(input) {
                ...
                const storePath = RED.settings.userDir ? RED.settings.userDir  // 3.2026
                                : "/opt/iobroker/iobroker-data/node-red/";
                this.cookieFile = fs.existsSync(this.cookieFile) ? this.cookieFile
                                : path.join(storePath, `alexa-cookie.json`);
                ...
                //  code advice copilot & new ver.5.0.58 oauth
                this.cookiePersistTimeout = null;
                this._persistCookieData = () => {    // new ver.5.0.58 oauth
                    if (this.authMethod !== 'proxy' || !this.cookieFile || !this.alexa || !this.alexa.cookieData) return;
                    const json = JSON.stringify(this.alexa.cookieData);
      ...
      this.ui.skills         = JSON.stringify([]);
        ...
        // ------------------------------------------------------------
        //  initAlexa() Fork – Token stabil, Proxy stabil
        // ------------------------------------------------------------
        this.initAlexa = async function(input, ignoreFile = false) {
         ...
         // ------------------------------------------------------------
                ...
                // ------------------------------------------------------------
                config.ignoreFile = ignoreFile;  // 02.2026 important!
                ...
                const tokenFile = path.join(storePath, `alexa-token.json`);  //-${this.id}.json`);
                ...
                switch (this.authMethod) {
                    case "token":
                        cookieData = await this.initToken(config, input);
                        break;

                    case "oauth":
                        cookieData = await this.initOAuth(config, input);
                        break;

                    default:
                        cookieData = await this.initDefault(config, input);
                        break;
                }
        ...
        }; // this.initAlexa 20260316

        this.initDefault = async function(config, input) {
            // ------------------------------------------------------------
            ...
            // orig. Version bbindreiter + try/catch + config.ignoreFile
                switch (this.authMethod) {
                    ...
                    //  const cookieData = tools.isObject(input) && input.loginCookie && tools.clone(input) || this.cookieFile && !config.ignoreFile && await readFileAsync(this.cookieFile, 'utf8').then(json => JSON.parse(json)).catch(this.warnCb) || undefined;
                        const cookieData = tools.isObject(input) && input.loginCookie && tools.clone(input) ||
                                this.cookieFile && !config.ignoreFile &&
                                await readFileAsync(this.cookieFile, "utf8")
                                .then(json => JSON.parse(json))
                                .catch(this.warnCb) || undefined;

                        config.cookieJustCreated = !cookieData;  // new ver.5.0.58 oauth
                ...
                if (this.authMethod === 'proxy' && this.cookieFile) {
                    this.persistCookieData();  // new ver.5.0.58 oauth
                }
        ...
        }; // based on this.initDefault v.5.0.58 oauth

        this.initToken = async function (config, input) {
        // --------------------------------------------------------
        // TOKEN-MODUS – nutzt Alexa2Auth Engine (auth.js)
        // --------------------------------------------------------
             let storedToken = null;

             config.proxyOnly = false;
         //  config.amazonPage = "amazon.de"; // 03.2026
         ...
         // Engine instanzieren
            const engine = new Alexa2Auth(this, config);

            this.cookieFile = tokenFile;
            this.attachAlexaHandlers();  // new ver.5.0.58 oauth, persists cookie in this.cookieFile
     ...
     /*
         // call Alexa-Instanz initExt() (node patch mit sanitizeSsml uvm)
     ...
     */
        ...
        this.initOAuth = async function(config, input) {
        // --------------------------------------------------------
        // OAuth-MODUS – nutzt AlexaAuthEngine (auth.js)
        // --------------------------------------------------------
            config.proxyOnly = false;
        //    config.amazonPage = "amazon.de"; // 03.2026
        //    config.baseAmazonPage = "amazon.de";

            this.setState("INIT_OAUTH");
            //const oauthNode = RED.nodes.getNode(config.oauth);
            //const Alexa2Auth = require("../lib/authEng/auth"); // => auth.js
            const oauthNode = require('../lib/oauth/oauth-config');  // => oauth-config.js

                //this.attachAlexaHandlers();  // new ver.5.0.58 oauth, persists token in this.cookieFile
            const cookieData = await oauthNode.getCookieData(this.tokenFile);

            await this.alexa.initExt(
                {
                cookie: cookieData,
                amazonPage: config.amazonPage,
                logger: config.logger,
...
// ENDE alexa-remote-account_v5058mod_raspi_20260316.js
